...
configs({ directions: ['ltr'], modes: ['ios'] }).forEach(({ title, config }) => {
  test.describe(title('button: form'), () => {
    ...
    test('should submit the closest form by pressing the `enter` button on a form element', async ({ page, }, testInfo) => {
    ...
    test('should submit the closest form with multiple elements by pressing the `enter` button', async ({ page, }, testInfo) => {
      testInfo.annotations.push({
        type: 'issue',
        description: 'https://github.com/ionic-team/ionic-framework/issues/19368',
      });
    ...
    test('should submit the form by id when form is set async', async ({ page }, testInfo) => {
      ...
      await button.evaluate((el) => (el.form = 'myForm'));
      await page.click('ion-button');
      expect(submitEvent).toHaveReceivedEvent();
    });
  });
  test.describe(title('should throw a warning if the form cannot be found'), () => {
    test('form is a string selector', async ({ page }) => {
      ...
      expect(logs[0]).toContain('[Ionic Warning]: Form with selector: "#missingForm" could not be found. Verify that the id is correct and the form is rendered in the DOM.');
    });
    test('form is an element reference', async ({ page }) => {
      ...
      `, config);
      expect(logs.length).toBe(1);
      expect(logs[0]).toContain('[Ionic Warning]: The provided "form" element is invalid. Verify that the form is a HTMLFormElement and rendered in the DOM.');
    });
  });
});
